...
def user_input(data):
    ...
    avg_trucks = st.sidebar.slider('Average Trucks', min_value=data['avg_trucks'].min(), max_value=100.0, value=0.0, step=0.5)
    avg_cars = st.sidebar.slider('Average Cars', min_value=data['avg_cars'].min(), max_value=100.0, value=0.0, step=0.5)
    avg_bikes = st.sidebar.slider('Average Bikes', min_value=data['avg_bikes'].min(), max_value=150.0, value=0.0, step=0.5)
    avg_pedestrians = st.sidebar.slider('Average Pedestrians', min_value=data['avg_pedestrians'].min(), max_value=200.0, value=0.0, step=0.5)
    v85 = data['v85'].mean()
    Telraam_data = 1
    Weather_data = 1
    # define LC_RAD60
